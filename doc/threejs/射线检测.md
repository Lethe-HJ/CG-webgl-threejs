在Three.js中，**射线拾取（Raycasting）**是3D交互的核心功能之一，用于检测场景中的物体是否被鼠标（或触摸）点击。其底层原理涉及几何数学和图形学的交叉计算。下面详细解释其实现机制：
...
### **总结**
Three.js的射线拾取本质是**将2D输入映射到3D空间，并通过几何计算实现物体选择**。其底层依赖：
1. **坐标转换**：屏幕坐标 → NDC → 世界空间射线。
2. **几何求交**：高效的射线-三角形/包围盒检测算法。
3. **优化策略**：层级剔除和按需检测平衡性能与精度。

理解这一原理后，可以扩展实现更复杂的功能（如拖拽、3D标注、碰撞检测等）。
