...
### 7.4 手写 Promise（符合 A+ 要点）

下面是一个满足上述规范要点的简易 Promise 实现（使用微任务队列，支持 then 链、值穿透与 Resolution 过程）。
...
MyPromise.resolve(1)
  .then()
  .then((v) => console.log('value:', v)); // value: 1

MyPromise.reject(new Error('err'))
  .then()
  .catch((e) => console.log('reason:', e.message)); // reason: err
