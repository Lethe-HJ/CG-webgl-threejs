...
- [浏览器事件循环 (Event Loop)](#浏览器事件循环-event-loop)
  - [目录](#目录)
  - [1. 什么是事件循环](#1-什么是事件循环)
  - [2. 任务队列](#2-任务队列)
...
Node.js 事件循环按**固定顺序**执行以下阶段（从上到下，循环执行）：

| 顺序 | 阶段 | 说明 | 常见 API |
|:---:|------|------|----------|
| 1 | **timers** | 执行到期的定时器回调 | `setTimeout`, `setInterval` |
| 2 | **pending callbacks** | 执行推迟到下一轮的系统操作回调 | 如 TCP 错误等 |
| 3 | **idle, prepare** | 内部使用 | - |
| 4 | **poll** | 执行 I/O 回调、等待新事件 | 文件、网络等 |
| 5 | **check** | 执行 setImmediate 回调 | `setImmediate` |
| 6 | **close callbacks** | 执行关闭相关回调 | 如 `socket.on('close')` |

> **注意**：每个阶段之间都会执行 `process.nextTick` 和 Promise 微任务。

**流程图**

![Node.js 事件循环流程图](./img/nodejs-event-loop.png)
