...
const vertexShaderSource = /*glsl*/ `
  ...
  uniform mat4 u_transform;//旋转矩阵
  void main() {
    gl_Position = u_transform * a_pos;

    // 光照处理
    // 顶点法向量进行矩阵变换，然后归一化
    vec3 normal = normalize((u_transform * a_normal).xyz);
    // 计算点光源照射顶点的方向并归一化
...
const canvas = document.getElementById("webgl-cube3");
...
function draw() {
  ...
  // 计算 4x4 矩阵 mxArr * myArr 并上传到 uniform
  const transformMatrix = multiplyMatrices(mxArr, myArr);

  // 把计算好的矩阵传给着色器的 uniform（注意：uniform 位置变量名为 transform）
